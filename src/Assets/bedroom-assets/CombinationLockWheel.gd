...
func _on_move_up_pointer_event(event):
	if event.event_type == XRToolsPointerEvent.Type.PRESSED:
		move_wheel_up()
		$AudioStreamPlayer3D.stream = load("res://src/assets/audio/sfx/VE_SFX_UI_Padlock_Turn_Var_01.ogg")
...
func _on_move_down_pointer_event(event):
	if event.event_type == XRToolsPointerEvent.Type.PRESSED:
		move_wheel_down()
		$AudioStreamPlayer3D.stream = load("res://src/assets/audio/sfx/VE_SFX_UI_Padlock_Turn_Var_02.ogg")
		$AudioStreamPlayer3D.play()
